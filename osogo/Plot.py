...
class BarPlot(Plot):
	...
	def __init__(self, owner, scale_type,root):
	    ...
	    #scale, databuffers, pixelheigth, boundaries
	    #draw x and y axis
	    self.zoomlevel=0
	    self.theOwner=root
	    self.yvaluemax_trigger_rescale=0.60 #if max(values) falls too low
	    self.yvaluemin_trigger_rescale=0.40
	...
	def addpoints(self,points):
	    #check if there was an excess of boundaries
	    #draw traces one by one
	    self.data_stack=points
	    self.getminmax()
	    if self.minmax[0]<self.yframe[0] or\
		self.minmax[1]>self.yframe[1]:
#		self.minmax[0]>self.yframe0max or\
#		self.minmax[1]<self.yframe1min:
		self.oldvalues={}
		self.reframey()
	    self.drawall()
	...
	def addtrace(self,add_list):
		...
		if pm!=None:
		    ...
		    #add to data_stack
		    self.data_stack[aFullPNString]=add_item[1][1]
	    #reframey
	    self.reframey()
	    #drawall from scratch
	    self.oldvalues={}
	    self.drawall()
	    return return_list
	...
	def getminmax(self):
		...
		a.append(self.data_stack[fpn])
	    #init values
	    if len(a)>0:
		self.minmax[0]=min(a) #minimum value of all
		self.minmax[1]=max(a) #maximum value of all
		for datapoint in a:
	    	    if datapoint>0:
			if self.minmax[4]>datapoint: 
			    self.minmax[4]=datapoint
	
	def drawall(self):
	    tracenumber=len(self.data_list)
	    columnwidth=self.plotarea[2]/(tracenumber*2+1)
	    if self.oldvalues=={}:
		#must be drawn from scratch
		self.clearplotarea()
		for fpn in self.data_list:
		    self.oldvalues[fpn]=self.origo[1]+1
	    #draw boxes
	    trace=0
	    for fpn in self.data_list:
		columnheigth=self.converty_to_plot(self.data_stack[fpn])
		oldcolumnheigth=self.oldvalues[fpn]
		difference=oldcolumnheigth-columnheigth
		if difference<0: 
		    brush="background" #delete column
		    y0=oldcolumnheigth
		    y1=-difference
		else:
		    brush=fpn
		...
		x0=columnwidth*(trace*2+1)+self.origo[0]
		self.drawbox(brush,x0,y0,columnwidth,y1)
		trace+=1
		self.oldvalues[fpn]=columnheigth
