...
template< typename T >
const Real stringToFloat( const String& aValue )
{
    try
    {
...
template<>
const Real stringCast<Real,String>( const String& aValue )
{
...
#if !HIGHREAL_IS_REAL
template<>
const HighReal stringCast<HighReal,String>( const String& aValue )
{
...
const char* getTempDirectory()
{
    ...
    _val = getenv("TMPDIR");
    if (_val && stat(_val, &sb) == 0 && (sb.st_mode & S_IFDIR)) {
        val = _val;
        return _val;
    }
