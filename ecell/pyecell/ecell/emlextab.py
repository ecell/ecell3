...
_lexre = '(?P<t_Stepper> Stepper[\\s|\\t] )|(?P<t_System> System[\\s|\\t] )|(?P<t_Process> Process[\\s|\\t] )|(?P<t_Variable> Variable[\\s|\\t] )|(?P<t_number> [+-]?(\\d+(\\.\\d*)?|\\d*\\.\\d+)([eE][+-]?\\d+)? )|(?P<t_fullid>[a-zA-Z]*:[\\w/\\.]*:\\w*)|(?P<t_identifier>[a-zA-Z_][a-zA-Z0-9_]*)|(?P<t_systempath>[a-zA-Z_/\\.][\\w/\\.]*)|(?P<t_quotedstrings> """[^"]*""" | \\\'\\\'\\\'[^\\\']*\\\'\\\'\\\' )|(?P<t_quotedstring> "(^"|.)*" | \\\'(^\\\'|.)*\\\' )|(?P<t_control> ^\\%line [^\\n]*\\n )|(?P<t_comment> \\# [^\\n]* )|(?P<t_nl> \\n+ )|(?P<t_whitespace> [ |\\t]+ )|(?P<t_RBRACE>\\})|(?P<t_LBRACE>\\{)|(?P<t_RBRACKET>\\])|(?P<t_RPAREN>\\))|(?P<t_LBRACKET>\\[)|(?P<t_LPAREN>\\()|(?P<t_SEMI>;)'
_lextab = [
  None,
  ('t_Stepper','Stepper'),
  ('t_System','System'),
