...
#        self.__clearCache()
    ...
    def createAttachment( self, aClass, anID ):
        """create an attachment"""
        aAttachmentElement = self.__createElement( 'attachment' )
        aAttachmentElement.setAttribute( 'class', aClass )
        aAttachmentElement.setAttribute( 'id', anID )
        
        self.__theDocument.documentElement.childNodes.append( aAttachmentElement )

    def deleteAttachment( self, anID ):
        """delete an attachment"""

        for anElement in self.__theEmlNode.childNodes:
            if anElement.nodeName == 'attachment' and \
                   anElement.getAttribute( 'id' ) == anID:
    ...
    def getAttachmentList( self ):
        ...
        aAttachmentNodeList = self.__getAttachmentNodeList()

        aAttachmentList = []
        for aTargetAttachmentNode in aAttachmentNodeList:

            aAttachmentID = aTargetAttachmentNode.getAttribute( 'id' )
            aAttachmentList.append( str( aAttachmentID ) )

        return aAttachmentList


    def getAttachmentPropertyList( self, aAttachmentID ):

        aAttachmentNodeList = self.__getAttachmentNode( aAttachmentID )
        aPropertyList = []

        for aChildNode in aAttachmentNodeList.childNodes:
    ...
    def getAttachmentProperty( self, aAttachmentID, aPropertyName ):

        aValueList = []

        aAttachmentNode = self.__getAttachmentNode( aAttachmentID )
        for aPropertyNode in aAttachmentNode.childNodes:
    ...
    def getAttachmentClass( self, aAttachmentID ):

        aAttachmentNode = self.__getAttachmentNode( aAttachmentID )
        return aAttachmentNode.getAttribute( 'class' )



    def __getAttachmentNodeList( self ):
        """private"""

        aAttachmentNodeList = []

        for aTargetNode in self.__theDocument.documentElement.childNodes:
            if aTargetNode.nodeName == 'attachment':
                aAttachmentNode = aTargetNode
                aAttachmentNodeList.append( aAttachmentNode )

        return aAttachmentNodeList


    def setAttachmentProperty( self, aAttachmentID, aPropertyName, aValue ):

        # what if a property with the same name already exist?
        aPropertyElement = self.__createPropertyNode( aPropertyName, aValue )
        aAttachmentNode = self.__getAttachmentNode( aAttachmentID )

        aAttachmentNode.appendChild( aPropertyElement )
        

    def __getAttachmentNode( self, aAttachmentID ):
        """private"""
        
        aAttachmentNodeList = self.__getAttachmentNodeList()

        for aTargetAttachmentNode in aAttachmentNodeList:
            if aTargetAttachmentNode.getAttribute( 'id' ) == aAttachmentID:
                
                return aTargetAttachmentNode
